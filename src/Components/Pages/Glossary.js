...
function Glossary() {
  ...
  const renderAction = useCallback(
    (isActiveEntry, sectionKey, entryKey, isAddingValue, disabled) => {
          ...
          <Button
            ...
            onClick={() => {
              ...
              if (isEmptyValue) {
                switch (sectionKey) {
                  case "basicFoods":
                    updates[`${basicFoodTagAssociationPath}/${entryKey}`] =
                      null;
                    ...
                    if (cookbook) {
                      Object.keys(cookbook).forEach((recipeId) => {
                        if (
                          cookbook[recipeId].ingredients.hasOwnProperty(
                            entryKey
                          )
                        ) {
                          updates[
                  ...
                  case "basicFoodTags":
                    if (basicFoodTagAssociation) {
                      Object.keys(basicFoodTagAssociation).forEach(
                        (basicFoodId) => {
                          if (
                            basicFoodTagAssociation[basicFoodId] === entryKey
                          ) {
                            updates[
  ...
  const memoInputButtonStack = useMemo(
    () => (sectionKey, entryKey) => {
    ...
    [glossary, editingEntry, getInputHandler, renderAction]
  );

  const getRenderInputButtonStack = (sectionKey) => (entryKey) =>
    memoInputButtonStack(sectionKey, entryKey);
